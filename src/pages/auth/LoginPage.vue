...
const disableSubmit = computed(() => !(member.username && member.password)); // form 유효성 검증(검사)

const login = async () => {
  //나중에 비동기 처리 위해서 async (await auth.login(member);) 부분.
  console.log(member);
  try {
    await auth.login(member);
    router.push('/'); //성공시 첫페이지로 이동
  } catch (e) {
    // 로그인 에러
    console.log('에러=======', e);
    error.value = e.response.data; //예외 발생 처리(에러에 문자열 저장) => 어떤 에러?? -> 유저아이디, 비밀번호가 일치하지 않습니다 같은..
  }
};
</script>
<template>
